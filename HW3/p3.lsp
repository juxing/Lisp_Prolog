...
; p3.lsp:    A Simple Puzzle Solver that can generate negative rules by background information.
;
;      - reads a series of puzzle descriptions of the form:
;             (  ( (a b ... ) (x y ...) )   ; puzzle symbols
;                 ((a (b c ...)) (d (e f ...)))  ; puzzle background
...
(defun gen_puz (puz_list)
    (let ( (sym_list (first puz_list)) )
         ...
         (setf *constraints* (rest puz_list))
         (cond
             ((= 2 (length *constraints*))  ; The dat file contains background.
                 (setf *constraints* (analyzer2 *constraints*))
             )

             ((= 1 (length *constraints*))  ; The dat file doesn't contain background.
                 (setf *constraints* (analyzer (first *constraints*)))
             )
         )
...
; This function is used to generate standard form of constraints from general form of constraints
; read from dat file.
(defun analyzer (constraints)
    (let (newconts)
        (setf newconts ())
        (dolist (cont constraints)
            (setf front (first cont))  ; Make the first element of a constraint a list.
            (cond
                ((not (listp front))
                    (setf front (list front))
                )
            )

            (setf end (second cont))  ; Make the second element of a constraint a list.
            (cond
                ((not (listp end))
                    (setf end (list end))
                )
            ...
            (dolist (1st front)  ; Go through the list and generate standard form of constraint pairs.
                (dolist (2st end)
                    (setf newbind (list 1st 2st))
                    (setf newconts (cons newbind newconts))
                )
            )
        )
        newconts
...
; This function is used to generate standard form of constraints based on background info read from
; dat file.
(defun analyzer2 (constraints)
    (let (background constrs newconstrs)
        (setf newconstrs ())
        (setf background (first constraints))  ; Read background info.
        (setf constrs (second constraints))  ; Read constraints.
        (setf constrs (analyzer constrs))  ; First generate standard constraints containing some background names.

        ; Replace background names with values in standard constraints.
        (dolist (cont constrs)
            (dolist (back background)
                (cond
                    ((position (first back) cont)
                        (setf cont (remove (position (first back) cont) cont))
                        (setf cont (cons (second back) cont))    
                    )
                )
            )

            (setf newconstrs (cons cont newconstrs))
        )

        ; Generate standard constraints again based on the new constraints based on background info.
        (setf newconstrs (analyzer newconstrs))
        newconstrs
    )
)
